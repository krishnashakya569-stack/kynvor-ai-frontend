  ...
  return (
    <div style={{ width:256, background:'#171717', display:'flex', flexDirection:'column', height:'100vh', borderRight:'1px solid #2a2a2a', flexShrink:0 }}>
      ...
      <button onClick={onNew}
        style={{ margin:'2px 10px 12px', display:'flex', alignItems:'center', gap:8, padding:'9px 12px', borderRadius:9, border:'1px solid #333', background:'transparent', cursor:'pointer', color:'#aaa', fontSize:13, transition:'all 0.15s', width:'calc(100% - 20px)' }}
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'0 6px' }}>
        {conversations.map(conv => (
          <button key={conv.id} onClick={() => onSelect(conv.id)}
            style={{ width:'100%', textAlign:'left', padding:'9px 12px', borderRadius:8, marginBottom:2, fontSize:13, border:'none', cursor:'pointer', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', transition:'all 0.12s',
              background: conv.id === activeId ? '#2a2a2a' : 'transparent',
              color: conv.id === activeId ? '#ececec' : '#888' }}
            onMouseEnter={e => { if(conv.id !== activeId) e.currentTarget.style.background='#222'; }}
